		...
		if data.get('result').get('action') == 'scene.activate':
			...
			if scene_to_activate:
				with AsyncTrackStates(hass) as changed_states:
					yield from hass.services.async_call('scene', SERVICE_TURN_ON, { ATTR_ENTITY_ID: 'scene.' + scene_to_activate}, True)
				result['data'] = self.json(changed_states)
			else:
				_LOGGER.warn('Could not process api.ai request: scene.activate called without scene name')

		for state in changed_states:
			_LOGGER.info('Changed state: %s', state)
			_LOGGER.info('Changed state: %s %s %s', state.entity_id, state.friendly_name, state.state)
